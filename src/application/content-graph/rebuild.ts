import {
  type GraphRebuildCheckpoint,
  graphRebuildCheckpointSchema
} from '@/core/graph/contracts'

export class GraphRebuildSession {
  ...
  constructor(input: {
    ...
    this.#checkpoint = input.checkpoint
      ? graphRebuildCheckpointSchema.parse(input.checkpoint)
      : graphRebuildCheckpointSchema.parse({
          id: `graph-rebuild:${input.generation}`,
    ...
    if (
      ...
      throw new TypeError('Graph rebuild checkpoint version mismatch')
    }
  }
  ...
  async run<T>(input: {
    ...
    this.#checkpoint = graphRebuildCheckpointSchema.parse({
      ...this.#checkpoint,
      state: 'running',
      updatedAt: input.at()
    })
    try {
      ...
      return { state, checkpoint: this.snapshot() } as const
    }
  }

  cancel() {
    this.#controller.abort(new Error('graph-rebuild-cancelled'))
  }
