...
describe('release workflow contracts', () => {
  ...
  it('applies the bundle limit to the Chrome release output', async () => {
    const source = await releaseScript('package.mjs')
    expect(source).toContain('guardBundleDirectory')
    expect(source).toContain("resolve(root, '.output', 'chrome-mv3')")
  })
  ...
  it('uses the protected Chrome store environment and never rebuilds', async () => {
  ...
  it('validates every store decision before it reaches the job output', async () => {
    ...
    expect(chromeJob).toContain('set -euo pipefail')
    expect(chromeJob).toContain('decision="$(jq')
    expect(chromeJob).toContain("jq -r '.decision' store-status.json")
    expect(chromeJob).toContain('already-present|blocked|eligible) ;;')
    expect(chromeJob).toContain('echo "decision=$decision" >> "$GITHUB_OUTPUT"')
    expect(chromeJob).not.toContain('echo "decision=$(jq')
  })
  ...
  it('audits egress before store credentials enter the publishing job', async () => {
    ...
    expect(chromeJob).toContain('egress-policy: audit')
    expect(chromeJob.indexOf(hardenRunner)).toBeLessThan(
      chromeJob.indexOf('actions/checkout@')
    )
  })
